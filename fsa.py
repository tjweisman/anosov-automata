...
class FSA:
    ...
    def in_dict(self):
        ...
        for v, neighbors_out in self._out_dict.iteritems():
            for w, labels in neighbors_out.iteritems():
                in_dict[w][v] = labels
        return in_dict

    def edges_out(self, vertex):
        for w, labels in self._out_dict[vertex].iteritems():
            for label in labels:
                yield (vertex, w, label)

    def edges_in(self, vertex):
        for w, labels in self._in_dict[vertex].iteritems():
            for label in labels:
    ...
    def edges(self):
        for v, nbrs in self._out_dict.iteritems():
            for w in nbrs:
                yield (v,w)
